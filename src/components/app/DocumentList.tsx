...
import { FileText, ImageIcon, ScanText, RefreshCw, Search, Eye, X, ExternalLink } from "lucide-react";
...
function PreviewModal({ doc, onClose }: { doc: Document; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Panel */}
      <div className="relative z-10 flex flex-col w-full max-w-4xl h-[85vh] rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden">
        {/* Header */}
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-slate-800/60 bg-slate-900/80 overflow-hidden">
          <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto_auto] items-center gap-4 border-b border-slate-800/60 px-4 py-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 w-8" />
          </div>

          <div className="divide-y divide-slate-800/40">
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="grid grid-cols-[1fr_auto_auto_auto_auto_auto_auto] items-center gap-4 px-4 py-3">
              ...
              filtered.map((doc) => (
                <div
                  key={doc.id}
                  className="grid grid-cols-[1fr_auto_auto_auto_auto_auto_auto] items-center gap-4 px-4 py-3 transition-colors hover:bg-slate-800/30"
                >
                  ...
                  <button
                    ...
                    className="flex size-7 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-700 hover:text-violet-400 transition-colors"
                    title="Vista previa"
                  >
